...
int main(void)
  {
    ...
    std::vector< point > pointListB; // Store points in vector so we can assign them to a polygon
    pointListB.push_back(point(1,1));
    pointListB.push_back(point(5,2));
    pointListB.push_back(point(5,0));
    pointListB.push_back(point(1,1));
    
    // create polygon
    polygon polB;
    // assign points to polygon
    boost::geometry::assign_points(polB, pointListB);
